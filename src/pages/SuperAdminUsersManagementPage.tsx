...
export function SuperAdminUsersManagementPage() {
  ...
  useEffect(() => {
    ...
    async function run() {
      try {
        ...
        const data = await searchUsers(search, { perPage: 10 })
        if (ignore) return
        setUsers(Array.isArray(data) ? data : [])

      } catch (caughtError) {
        if (ignore) return
        setError(caughtError instanceof Error ? caughtError.message : 'Impossible de charger les utilisateurs.')
      } finally {
    ...
    <div className="space-y-6">
      <div>
        <h1 className="flex items-center gap-3 text-2xl font-bold">
          ...
          Gestion du Personnel (Super Admin)
        </h1>
