  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        if (!ignore) setOrganizations(Array.isArray(data) ? data : [])
      } catch (caughtError) {
        if (!ignore) setError(getApiErrorMessage(caughtError, 'Impossible de charger les organisations.'))
      } finally {
...
const payload = {
        ...
        ...(form.email.trim() ? { email: form.email.trim() } : ({} as { email?: string })),
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-2 text-sm text-slate-500">Créer / modifier / supprimer des organisations.</p>
      </div>
      ...
      <section className="rounded-md border border-slate-200 bg-white p-4">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-slate-500">Détails + actions.</p>
          </div>
